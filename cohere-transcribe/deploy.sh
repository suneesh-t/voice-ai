...
# Disable AWS CLI pager to prevent output from hanging
export AWS_PAGER=""

# ============================================================
# Deploy CohereLabs/cohere-transcribe-03-2026 on EKS
# Uses vLLM as the production inference engine via Ray Serve
#
# Prerequisites:
#   - aws CLI configured with correct credentials
#   - kubectl configured for model-management cluster
#   - docker installed and running
#   - HuggingFace token with access to the gated model
# ============================================================

REGION="us-east-1"
CLUSTER_NAME="model-management"
AWS_ACCOUNT_ID=$(aws sts get-caller-identity --query Account --output text)
ECR_REPO="cohere-transcribe-serve"
...
echo "=========================================="
echo "Deploying cohere-transcribe (vLLM) to EKS"
...
sed -e "s|<AWS_ACCOUNT_ID>|${AWS_ACCOUNT_ID}|g" \
    ...
    "${SCRIPT_DIR}/ray-service-transcribe.yaml" | kubectl apply -f -

echo -e "\n=========================================="
echo "Deployment initiated! (image tag: ${IMAGE_TAG})"
echo ""
echo "Monitor progress:"
echo "  kubectl get rayservice cohere-transcribe -w"
echo "  kubectl get pods -l ray.io/cluster -w"
echo ""
echo "Once running, port-forward to test:"
echo "  kubectl port-forward svc/cohere-transcribe-serve-svc 8000"
echo ""
echo "Test with:"
