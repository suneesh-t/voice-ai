...
MODEL_REVISION = "76b8b23e8607f35f0265a23d481b338fb0e26aea"
VLLM_PORT = 8100
...
class CohereTranscribeVLLM:
    ...
    def _wait_for_server(self, timeout: int = 300):
        ...
        while time.time() - start < timeout:
            if self.process.poll() is not None:
                ...
                raise RuntimeError(f"vLLM process exited early (code {self.process.returncode}): {output[-2000:]}")
            try:
        ...
        try:
            ...
            if stream:
                return StreamingResponse(
                    self._stream_transcribe(audio_bytes, filename),
    ...
    async def _stream_transcribe(self, audio_bytes: bytes, filename: str):
