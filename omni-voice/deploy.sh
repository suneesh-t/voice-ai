...
#
# Deploy OmniVoice TTS to EKS (model-management cluster).
#
# Steps:
...
ECR_REPO="omni-voice-serve"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

export AWS_PAGER=""

# Image tag: <git-short-sha>-<timestamp>
...
sed -e "s|<AWS_ACCOUNT_ID>|${AWS_ACCOUNT_ID}|g" \
    ...
    "${SCRIPT_DIR}/ray-service-omnivoice.yaml" | kubectl apply -f -
...
echo ""
echo "Monitor:"
echo "  kubectl get rayservice omni-voice-service"
...
echo "  kubectl port-forward svc/omni-voice-service-serve-svc 8002:8000"
echo "  curl -X POST http://localhost:8002/v1/audio/speech \\"
echo "    -H 'Content-Type: application/json' \\"
echo "    -d '{\"input\":\"Hello world\",\"response_format\":\"wav\"}' \\"
