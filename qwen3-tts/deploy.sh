...
# ============================================================
# Deploy Qwen/Qwen3-TTS-12Hz-1.7B-CustomVoice on EKS
# Uses vllm-omni as the production inference engine via Ray Serve
#
# Prerequisites:
...
echo "=========================================="
echo "Deploying qwen3-tts (vllm-omni) to EKS"
...
echo -e "\n[5/6] Creating HuggingFace token secret..."
if [ -z "${HF_TOKEN:-}" ]; then
...
sed -e "s|<AWS_ACCOUNT_ID>|${AWS_ACCOUNT_ID}|g" \
    ...
    "${SCRIPT_DIR}/ray-service-tts.yaml" | kubectl apply -f -

echo -e "\n=========================================="
echo "Deployment initiated! (image tag: ${IMAGE_TAG})"
echo ""
echo "Monitor progress:"
echo "  kubectl get rayservice qwen3-tts -w"
echo "  kubectl get pods -l ray.io/cluster -w"
echo ""
echo "Once running, port-forward to test:"
echo "  kubectl port-forward svc/qwen3-tts-serve-svc 8000"
echo ""
echo "Test with:"
echo '  curl -X POST http://localhost:8000/v1/audio/speech \'
echo '    -H "Content-Type: application/json" \'
echo '    -d '"'"'{"input":"Hello, how are you?","voice":"vivian","language":"English"}'"'"' \'
echo '    --output output.wav'
echo ""
echo "Streaming test:"
echo '  curl -X POST http://localhost:8000/v1/audio/speech \'
echo '    -H "Content-Type: application/json" \'
echo '    -d '"'"'{"input":"Hello, how are you?","voice":"vivian","stream":true,"response_format":"pcm"}'"'"' \'
echo '    --no-buffer | play -t raw -r 24000 -e signed -b 16 -c 1 -'
echo "=========================================="
